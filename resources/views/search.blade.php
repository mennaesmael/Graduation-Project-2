            ...
            <form action="{{ route('search') }}" method="GET" class="flex items-center justify-center mt-2">
                ...
                <div class="flex w-full space-x-2 md:w-auto">
                    <input
                        ...
                        type="text" name="query" placeholder="البحث عن ملف..." aria-label="Search by file name" style="width: 400px;" id="searchInput" list="suggestionsList">
                            ...
                            <table class="min-w-full leading-normal">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                     ...
                                     <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                      <span class="font-medium text-gray-800">{{ $file->model()->file_id }}</span>
                                     </td>
                                     <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                      <span class="font-medium text-gray-800">{{ $file->model()->user_id }}</span>
                                     </td>
                                     <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                      <span
                                    ...
                                    class="font-medium text-gray-800">{{ $file->model()->file_size }}</span>
                                       ...
                                       <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                     <span class="font-medium text-gray-800">{{ $file->model()->notes }}</span>
                                       </td>
                                       <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                     <span
                                     ...
                                     @endif
                                       ...
                                       <td class="px-3 py-3 border-b border-gray-200 bg-white text-sm">
                                     @if ($file->model()->file_id)
                                         <form action="{{ route('delete', $file->model()->file_id) }}"
                                       method="POST">
                                       @csrf
                                       @method('DELETE')
                                       <button type="submit"
...
<script>
    ...
    fetch(`http://127.0.0.1:8001/suggestions?query=${encodeURIComponent(query)}`)        .then(response => response.json())
        .then(suggestions => {
            datalist.innerHTML = '';
